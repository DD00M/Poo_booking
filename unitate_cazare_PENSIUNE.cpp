...
unitate_cazare_PENSIUNE::unitate_cazare_PENSIUNE() :unitate_cazare()
{
}

unitate_cazare_PENSIUNE::unitate_cazare_PENSIUNE(std::string nume, std::string oras, int clasificare, std::vector<std::string>lista, std::vector<std::string> mese, std::vector<std::string> preturi) : unitate_cazare(nume, oras, clasificare,lista, mese, preturi)
{
}

unitate_cazare_PENSIUNE::unitate_cazare_PENSIUNE(const unitate_cazare_PENSIUNE& src) : unitate_cazare(src)
{
}

unitate_cazare_PENSIUNE::~unitate_cazare_PENSIUNE()
{
}

void unitate_cazare_PENSIUNE::print_cazare()
{
	std::cout << "Pensiunea cu numele: " << this->m_denumire << " din orasul: " << this->m_oras << std::endl;
	std::cout << "---------Caracteristicile pensiunii---------" << std::endl;
	if (this->m_clasificare == 1) {
		std::cout << "Pensiunea are " << this->m_clasificare << " margarete" << std::endl;
	}
	else {
		std::cout << "Pensiunea are " << this->m_clasificare << " margarete" << std::endl;
	}
	...
	std::cout << "<------------------------------------------------------------------------------------------------------------------------------------->" << std::endl;
	std::cout << "MESELE PE CARE PENSIUNEA LE POATE OFERI CLIENTILOR: " << std::endl;
	...
	std::cout << "<------------------------------------------------------------------------------------------------------------------------------------->" << std::endl;
	std::cout << "LISTA CU DATELE DEJA OCUPATE DE CATRE CLIENTII NOSTRII(observatie: nu veti putea selecta o zi deja ocupata, pentru a nu fi nevoiti sa reluati cererea de rezervare, va rugam sa completati lista de zile cu atentie maxima!\n";
	for (int i = 0; i < this->m_lista_date_rezervari.size(); i++) {
		this->m_lista_date_rezervari[i]->print_zi();
	}
	std::cout << "<------------------------------------------------------------------------------------------------------------------------------------->" << std::endl;
	if (this->m_lista_recenzii.size() == 0) {
		std::cout << "Pensiunea " << this->m_denumire << " nu a primit inca recenzii" << std::endl;
	}
	else {
		...
		std::cout << "RECENZIILE PENSIUNII PRIMITE DIN PARTEA CLIENTILOR: " << std::endl;
